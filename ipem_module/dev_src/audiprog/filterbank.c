 ...
 *******************************************************************/
{
  ...
  for (m = 1; m <= ncel; m++)
  {
    resp = resp * ((pow(1 + bpfd->cell[m].a1 * c1 + bpfd->cell[m].a2 * c2, 2.0) + pow(bpfd->cell[m].a1 * s1 + bpfd->cell[m].a2 * s2, 2.0)) / (pow(1 + bpfd->cell[m].b1 * c1 + bpfd->cell[m].b2 * c2, 2.0) + pow(bpfd->cell[m].b1 * s1 + bpfd->cell[m].b2 * s2, 2.0)));
  }
  return resp;
}
...
#define duc2 0.64
  ...
  if (fs == fsmp / step[1])
    ...
    r = 2;
  r = 1 - exp(-0.25 * uc * r);
  bpfd->cell[ncel].a1 = -2 * r;
  bpfd->cell[ncel].a2 = r * r;
...
void write_filterbank()
{
  ...
  if (open_writefile("filters.dat"))
  {
    ...
    for (i = 1; i <= 200; i++)
    {
      ut = i * umax / 200;
      f = umin1(ut);
      fprintf(writefile, "%7.3f%7.3f", f, ut);
      for (p = 1; p <= nchan; p++)
      {
        fs = fsmp / step[p];
        if (f >= 0.5 * fs)
        ...
        {
          y = pow(bpfd[p].gain, 2.0) * h2(f, fs, &bpfd[p]);
          y = 4.34 * log(y + 1.0E-08);
        }
...
*/
  {
    ...
    for (m = 1; m <= ncel; m++)
    {
      x = y - bpfd[p].cell[m].b1 * bpfd[p].cell[m].w1 - bpfd[p].cell[m].b2 * bpfd[p].cell[m].w2;
      y = x + bpfd[p].cell[m].a1 * bpfd[p].cell[m].w1 + bpfd[p].cell[m].a2 * bpfd[p].cell[m].w2;
      bpfd[p].cell[m].w2 = bpfd[p].cell[m].w1;
      bpfd[p].cell[m].w1 = x;
    }
    ybpf[p] = y;
  }
