...
void startup_sigio()
{
        ...
        rx = 127 / log(1 + mu);
        for (i = 0; i <= 127; i++)
                etab[i] = (exp(i / rx) - 1) / mu;
        for (i = 0; i <= 2047; i++)
                ctab[i] = round_int(rx * log(1 + mu * i / 2047.0));
        etab[128] = etab[127];
        ctab[2048] = ctab[2047];
}
...
void read_header()
{
        ...
        fread(&c4, 1, 4, readfile); /* 1 */// (4) 1. Magic word: "RIFF"
        fread(&c4, 1, 4, readfile); /* 2 */// (4) 2. length of the file, after the magic word
        fread(&c4, 1, 4, readfile); /* 3 */// (4) 3. Magic word 2: "wave"
        fread(&c4, 1, 4, readfile); /* 4 */// (4) 4. Magic word 3: "fmt "  (=format)
        fread(&c4, 1, 4, readfile); /* 5 */// (4) 5. length of the subsequent format header
        fread(&c2, 1, 2, readfile); /* 6 */// (2) 6. filetype, 10 is PCM
        fread(&c2, 1, 2, readfile); /* 7 */// (2) 7. nr of channels, 10 is mono
        fread(&c4, 1, 4, readfile); /* 8 */// (4) 8. sampling rate in Hz, e.g. 0h 0h 56h 22h is 22050 Hz

        printf("sampling frequency: %d Hz\n", c4[0] + 256 * (c4[1] + 256 * (c4[2] + 256 * (c4[3]))));

        fread(&c4, 1, 4, readfile); /* 9 */// (4) 9. average number of bytes per second
        fread(&c2, 1, 2, readfile); /* 10 */// (2) 10. block align (?)
        fread(&c2, 1, 2, readfile); /* 11 */// (2) 11. sample width
        ...
        fread(&c4, 1, 4, readfile); /* 12 */// (4) 12. Magic word: "data"
        fread(&c4, 1, 4, readfile); /* 13 */// (4) 13. length of the subsequent datablok
...
double new_sample(int *last)
{
        if (binary)
                return one_binary_sample(last);
        if (wavefiles)
                return one_wave_sample(last);
}
...
void write_sample(int last, double x)
{
        int ixn;
        ...
        tbuf[write_ptr + 2] = (ixn / 64) + '0';
        write_ptr += 2;
        if ((last) || (write_ptr >= 66))
                write_new_record();
}
