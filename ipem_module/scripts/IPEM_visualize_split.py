...
def visualizer():
    ...
    def setup_objects():
        ...
        pos[1, :, :, 1] += (- Y_SPLIT_SPACING - N_SUBCHANNELS * Y_SPACING)
    ...
    def ani_cb(data):        
        ...
        clr[0, :, :, 3] = np.clip(np.array(data.left_channel).reshape( [CHUNK_SIZE, N_SUBCHANNELS]) * 3, 0., 1.)
        clr[1, :, :, 3] = np.clip(np.array(data.right_channel).reshape([CHUNK_SIZE, N_SUBCHANNELS]) * 3, 0., 1.)
